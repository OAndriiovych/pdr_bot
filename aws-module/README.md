...
```bash
aws-module$ aws s3 mb s3://BUCKET_NAME
```

To prepare the application for deployment, use the `sam package` command.

```bash
aws-module$ sam package \
...
```bash
aws-module$ sam deploy \
    --template-file packaged.yaml \
    --stack-name aws-module \
...
```bash
aws-module$ sam local invoke HelloWorldFunction --event events/event.json
```
...
```bash
aws-module$ sam logs -n HelloWorldFunction --stack-name aws-module --tail
```
...
```bash
aws-module$ cd HelloWorldFunction
...
```bash
aws-module$ sam delete --stack-name aws-module
aws-module$ aws s3 rb s3://BUCKET_NAME
```
